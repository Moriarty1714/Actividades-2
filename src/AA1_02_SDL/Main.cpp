...
int main(int, char*[])
{
	// --- INIT SDL ---																																		//Constructor de la librería (ya que està en C++)
	...
	//-->SDL_Image 
	const Uint8 imgFlags{ IMG_INIT_PNG | IMG_INIT_JPG };																					// Formatos de imàgen a trabajar (lo de dentro son enum) Binari
	...
	while (isRunning) {
		...
		while (SDL_PollEvent(&event)) {											//Cada vez que entres en el while, dame un evento.
			switch (event.type) {
			case SDL_QUIT:														//Botón de cruz de salida
				isRunning = false;
				break;
			case SDL_KEYDOWN:
